...
upperLim=lowerLim+5; %replace with length(files) for last file
    ...
    while(countOnes==0 && boxSize>0)
        ...
        for i=1:2*boxSize:(size(resized_image,1))
            for j=1:2*boxSize:(size(resized_image,2))
                ...
                randPoints=[rand(10,1)*2*boxSize;rand(10,1)*2*boxSize];
        ...
        boxSize=boxSize/2; % half box size for concentrated fixation distribution
    end
