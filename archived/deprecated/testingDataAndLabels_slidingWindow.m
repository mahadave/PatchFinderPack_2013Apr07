function [pointList,trainingLabels,patchInfo] = testingDataAndLabels_slidingWindow(trainingLimits,NPoints)
...
%        [resized_image,densityFix]=getFixationKDE(image,users,filename);
        ...
        %
         ...
         shift=1.5*boxSize; % 5 pixel shift
         for i=1:shift:(size(resized_image,1))
            for j=1:shift:(size(resized_image,2))
                ...
                if(meanDensity>=probThreshold)
                  ...
                  patchInfo(imgIndex).patchData(patchIndex).positivePixels = pixelLoc;
                       ...
                       patchInfo(imgIndex).boxSize = boxSize;
                                          patchInfo(imgIndex).shift = shift;
                       patchInfo(imgIndex).centerCoords = [lowerX+boxSize lowerY+boxSize];
...
%       disp([SX SY]);
       patchInfo(imgIndex).interPatchHist = getINTERPatchDistHistogram_v3(patchInfo(imgIndex),[SX SY]);
         end
