    function [ListSet,patchInfo] = validatingDataAndLabels_v13_intraPatchInterPatch(META)
    % Training limits e.g. [1 50] .. i.e. img 1 to img 50
    % NPoints e.g. [10] i.e. number of points to extract from each patch - it
    % is an important parameter
    
    
    tic; [ListSet,patchInfo] = trainingDataAndLabels_v14_intraPatchInterPatch(META,META.validationLimits); toc;
...
%{    
    ...
    debug=META.debug;
    users=META.users;
    lowerLim =  META.validationLimits(1);
    upperLim =  META.validationLimits(2);
  ...
  %                     disp('pi = '); disp(patchIndex);
         ...
         if META.USE_SELFSIM==1
            ...
            [SX SY] = size(resized_image);
             ...
             disp(META.NBins)
            patchInfo(imgIndex).interPatchHist = getINTERPatchDistHistogram_v5(patchInfo(imgIndex),[SX SY],META.NBins);
         end
