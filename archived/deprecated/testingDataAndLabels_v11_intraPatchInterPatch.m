function [pointList,trainingLabels,patchInfo] = testingDataAndLabels_v11_intraPatchInterPatch(trainingLimits,NPoints)
...
%        [resized_image,densityFix]=getFixationKDE(image,users,filename);
        ...
        %
         ...
         for i=1:2*boxSize:(size(resized_image,1))
            ...
            [MX MY]=size(rgb2gray(resized_image));
  ...
  %                     disp('pi = '); disp(patchIndex);
                ...
                d_all = [];
                kaalaPatch = rgb2gray(patch);
                [FX,FY] = gradient(double(kaalaPatch));
                u = (abs(FX/2) + abs(FY/2));
                ...
                kaalaPatch=rgb2gray(patch);
                [f,d] = vl_sift(single(kaalaPatch),'frames',fc) ;
                d_all = [d_all; d'];
         ...
         if ss==1
            [SX SY] = size(rgb2gray(resized_image));
%       disp([SX SY]);
