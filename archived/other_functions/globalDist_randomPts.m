function brayCurtisSim=globalDist_randomPts (imageIndex)
...
mainImg=0;
    ...
    for L=1:size(FixPoints,1)
        randomPts = [randomPts; [(Xlim-0.05*Xlim)*rand(1) (Ylim-0.05*Ylim)*rand(1)]];
    end
    
    siftPoints = double(round((randomPts)));
    
    if (show==1 || mainImg==1)
       
        for k = 1:length(randomPts)
            text ((randomPts(k, 1)), (randomPts(k, 2)), ['{\color{red}\bf', num2str(k), '}'], 'FontSize', 3, 'BackgroundColor', 'g');
        end
