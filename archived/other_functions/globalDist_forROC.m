function brayCurtisList =globalDist_forROC(pt,et)
...
upperLim=lowerLim+10;%length(files); %replace with length(files) for last file
...
for i = lowerLim:upperLim
    ...
    if(size(siftPoints,1)<=10) % for no sift points
        chi_measure =2; 
        bcMeasure = 2;
